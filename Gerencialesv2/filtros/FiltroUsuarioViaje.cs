...
{
    public partial class FiltroUsuarioViaje : Form
    {
        public frmPrincipal principal;
        Conexion con = new Conexion();
        public FiltroUsuarioViaje()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Hide();
                reportes.frmUsuarioViaje rpt = new reportes.frmUsuarioViaje();
                string value = ((KeyValuePair<string, string>)emp.SelectedItem).Value;
                principal.crearRT4(Convert.ToDateTime(fechaIni.Text), Convert.ToDateTime(fechaFin.Text), rpt, int.Parse(emp.SelectedValue + ""), value);
            }
            else
